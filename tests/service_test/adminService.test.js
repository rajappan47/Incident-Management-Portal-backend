const mongoose = require('mongoose'); // Required for Mongoose ID validation
...
const createUser = async ({ name, email, password, role }) => {
  if (!name || !email || !password) {
    // Explicit 400 Bad Request for missing required fields
...
const createCategory = async ({ name, description }) => {
  if (!name) {
    // Explicit 400 Bad Request if category name missing
    throw createCustomError('Category name is required', 400);
  }

  // FIXED: Create the category FIRST so newCategory is defined for the logger
